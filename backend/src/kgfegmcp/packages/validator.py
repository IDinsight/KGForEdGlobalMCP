...
Validation in this module remains curriculum-agnostic. Local statement types, parent
rules, code conventions, subjects, grades, languages, and rights are read from the
selected immutable profile. Delivery-schema 1.0 behavior is expressed through shared
wire constants, including the generic ``unresolvedRootFallback`` exception.

The validator uses only temporary local dictionaries and sets. It does not expose a
reusable graph store, traversal API, catalog, search service, FastMCP application, MCP
...
from kgfegmcp.profiles.models import StatementTypePolicy
    ...
    if (
        ...
        findings.append(
            _finding(
                ...
                message="Decoded statement-code evidence conflicts with the selected profile.",
            )
        )
    elif (
        ...
        findings.append(
            _finding(
                ...
                message="Decoded statement-code evidence is incomplete for the selected profile.",
            )
        )
    elif (
        ...
        findings.append(
            _finding(
                ...
                message="Decoded statement-code evidence is absent for a partial-code profile.",
            )
        )
...
def _validate_cycle_and_reachability(
    *,
    ...
    while queue:
        ...
        targets = adjacency[source_id] if source_id in adjacency else []
...
def _validate_item_semantics(
    *, package: LoadedGraphPackage, findings: list[PackageValidationFinding]
    ...
    for node in package.item_nodes:
        ...
        if node.grade_level is not None:
            if not node.grade_level or len(node.grade_level) != len(
                set(node.grade_level)
            ):
                findings.append(
                    _finding(
            ...
            if statement_code in observed_codes:
                findings.append(
                    _finding(
                        ...
                        details={
                            "first_node_id": str(
                                observed_codes[statement_code].node_id
                            ),
...
def _validate_parent_policy(
    *,
    ...
    for target in package.item_nodes:
        ...
        unexpected_status_relationships = [
            relationship
            for relationship in incoming
            if relationship.resolution_status is not None
        ]

        for relationship in unexpected_status_relationships:
            findings.append(
                _finding(
        ...
        if statement_type in root_statement_types:
            framework_parent_count = sum(
                str(relationship.source_node_id) == root_id
        ...
        for parent_type, count in counts_by_parent_type.items():
