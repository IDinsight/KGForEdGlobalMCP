...
**Status:** Phase 0 foundational contracts, PR 2 JSONL boundary, and PR 3 deterministic manifest construction implemented; PR 4 package loading and validation is next  
...
Phase 0, PR 2, and PR 3 have been implemented under
...
- typed `JSONLParsingError`, `DeliveryPropertyDecodingError`, and
  `ManifestBuildError` failures with safe public messages;
- repository-owned `SOURCE_SCHEMA_VERSION = "1.0"` and
  `DELIVERY_SCHEMA_VERSION = "1.0"` compatibility constants;
- exact-byte SHA-256 helpers and canonical immutable artifact-set hashing;
- strict profile resolution beneath the configured profile root;
- immutable `PackageBuildSpec` and `PackageBuildResult` construction contracts;
- deterministic pending package construction through the existing profile, decoder,
  manifest, and identifier contracts;
...
- a Typer-based manifest-build CLI supporting specification-file, explicit-input, and
  dry-run modes;
- six generated candidate packages and `package_manifest.json` files using the selected
  profiles and accepted `as_*` artifacts.
...
PR 2 and PR 3 verification covered all current delivery fixtures:
...
each of the six packages. PR 3 manifest counts are:
...
The generated packages preserve the accepted source bytes and `as_` filenames, use safe
package-relative paths, and remain at `validation.status="pending"` with
`validatedAt=null`.

No complete manifest repository, immutable loaded-package aggregate, package-wide
validator, validation CLI, DAG graph store, traversal service, catalog, search service,
FastMCP application, resource layer, or prompt layer has been implemented yet.

Automated tests were intentionally deferred for Phase 0, PR 2, and PR 3 at the
operator's request. They remain required before the affected behavior is
production-ready. Until the operator requests tests, each implementation unit must run
deterministic formatting, linting, type checking, compilation, and representative
...
packages/checksums.py
    Owns exact-byte SHA-256 calculation and canonical artifact-set hashing. It does not
    decode, rewrite, normalize, or interpret artifact content.

packages/builder.py
    Coordinates construction-time profile resolution, decoding, metadata agreement,
    counts, capabilities, checksums, identities, staging, and pending manifest
    materialization. It does not perform PR 4 package-wide acceptance validation.

profiles/loader.py
    Resolves and validates one selected profile beneath the configured profile root and
    calculates the exact profile-file checksum. It is not yet the full startup profile
    repository.

cli/build_manifests.py
    Provides the Typer entry point for specification-file, explicit-input, and dry-run
    manifest builds. Domain work remains in ordinary package services.
...
their exact checksum mapping remain protected by the manifest and PR 4 validation.
...
the validation status. `approved` is not a package validation status. PR 4 owns the
controlled transition from `pending` to `passed`, `failed`, or `quarantined` and must
set `validatedAt` without altering `createdAt` or package-defining fields.
...
PR 4 implements the package loading and package-wide validation foundation used by
later startup behavior. FastMCP lifespan wiring remains a later phase.
...
PR 4 validation begins from an immutable candidate package with
`validation.status="pending"`. A persisted terminal result must use exactly `passed`,
`failed`, or `quarantined`, set `validatedAt`, retain `createdAt`, and leave every
package-defining field and artifact byte unchanged. `approved` is not a supported
validation status.
...
PR 4 should use `PackageValidationError` and `ProfileValidationError` for package- and
...
conversion. PR 3 completed deterministic pending package construction. PR 4 package
loading and package-wide validation is the next unit.
...
- exact-byte checksum and canonical artifact-set hash services;
- strict selected-profile loader with exact profile checksum calculation;
- deterministic manifest builder driven by versioned profiles and accepted `as_*`
  artifacts;
- Typer manifest-build CLI with specification-file, explicit-input, and dry-run modes;
- staged, verified, atomic, and idempotent pending package materialization;
- six generated candidate package fixtures with pending manifests.

### Remaining deliverables

- manifest repository and safe package discovery;
- full profile repository for startup and package validation;
- checksum verification during package loading;
- immutable loaded-package aggregate;
- package-wide validator and Typer validation CLI;
- controlled pending-to-terminal validation transition;
- detailed-artifact path verification during loading;
- DAG-aware graph store;
- catalog repository.
...
3. **PR 3 — implemented:** deterministic pending package construction from versioned
   profiles and accepted `as_*` delivery, detailed, and explicitly named additional
   artifacts; exact checksums and canonical identities; Typer build CLI; and six
   generated package fixtures.
4. **PR 4 — next:** manifest and profile repositories, package checksum verification,
   immutable loaded-package assembly, package-wide validation, controlled validation
   status transition, and a Typer validation CLI.
...
- reuses `packages/decoder.py` as the only delivery JSONL and property-decoding
  boundary;
- resolves the selected profile beneath the configured profile root and verifies its
  exact bytes;
- populates source and delivery schema versions automatically from repository-owned
  `1.0` constants;
- supports exactly package revision 1;
- compares deterministic profile and framework-root overlaps and fails on material
  disagreement;
- calculates framework, item, relationship, coded-item, unresolved-relationship, and
  multi-parent-target counts;
- recognizes only the seven approved detailed artifact basenames;
- packages unknown artifacts only through explicit unique logical names;
- derives capabilities from profile policy and actual decoded evidence;
- calculates exact artifact checksums and the canonical immutable artifact-set hash;
- generates snapshot and graph-package identifiers through the existing identifier
  builders;
- preserves exact artifact bytes and original safe `as_` filenames;
- writes only safe package-relative paths;
- constructs the existing `GraphPackageManifest`;
- materializes candidate packages through a verified staging directory and atomic
  rename;
- returns `existing_identical` without writes for an equivalent pending package;
- rejects conflicting or terminal destinations and provides no force-overwrite option;
- keeps validation pending and leaves the terminal transition to PR 4;
- exposes a Typer CLI without a required subcommand.

PR 3 intentionally did not add the complete manifest repository, package startup
loading, quarantine policy, graph-wide endpoint validation, cycle or reachability
acceptance validation, DAG graph storage, traversal, catalog, search, FastMCP behavior,
resources, prompts, or automated tests.

### Next implementation unit: PR 4 package loading and validation

PR 4 must turn a constructed candidate directory into a safely loaded and fully
validated immutable package aggregate. It must remain separate from the graph store,
catalog, search, FastMCP, resources, and prompts.
...
backend/src/kgfegmcp/errors.py                    # only for concrete typed gaps
backend/src/kgfegmcp/packages/models.py           # only for load/validation aggregates
backend/pyproject.toml                            # Typer entry point/dependency as needed
```

Do not create graph adjacency indexes or catalog services in PR 4. Repository naming may
be adjusted after inspecting the current source, but responsibilities must remain
separate: discovery, loading, validation, and controlled status persistence.

#### PR 4 responsibilities

1. Discover candidate package directories only beneath the configured graph-packages
   root. Do not accept arbitrary manifest-provided filesystem paths.
2. Read `package_manifest.json` through the existing `GraphPackageManifest` model.
3. Resolve the referenced profile by ID and version beneath the configured profile root
   and require its exact checksum to match the manifest.
4. Resolve every declared artifact path safely beneath the package root and reject
   absolute paths, traversal, backslashes, symlinks, symlink escape, undeclared content,
   missing files, duplicate destinations, and case-insensitive collisions.
5. Require exact checksum coverage for every declared artifact and verify checksums from
   exact packaged bytes.
6. Decode delivery artifacts through the existing PR 2 decoder.
7. Assemble an immutable loaded-package aggregate containing manifest context, selected
   profile, decoded framework root, decoded item nodes, decoded relationships, and safe
   declared artifact references.
8. Perform complete package-wide validation, including:
   - exactly one framework root;
   - supported node and relationship labels;
   - outer/property identifier agreement required by delivery schema 1.0;
   - unique node identifiers;
   - unique relationship identifiers;
   - duplicate endpoint-pair policy;
   - resolvable endpoints;
   - endpoint-label agreement;
   - property-declared endpoint agreement with outer endpoints;
   - self-loop rejection unless explicitly permitted;
   - directed `hasChild` cycle rejection;
   - expected root reachability;
   - preservation of every valid parent edge;
   - explicit relationship-status vocabulary;
   - manifest count agreement;
   - manifest capability agreement;
   - framework metadata and profile-semantic agreement;
   - normalized statement-type compatibility;
   - detailed validation-report acceptance when declared.
9. Produce structured validation findings with stable codes, severity, safe public
   messages, and internal diagnostic details.
10. Support validation of one package and all discovered pending packages through a
    Typer CLI, including a read-only mode that does not persist status.
11. Own the controlled one-way transition from `pending` to exactly one terminal status:
    `passed`, `failed`, or `quarantined`.
12. Set `validatedAt` exactly once when persisting a terminal state and never modify
    `createdAt`.
13. Preserve package identity, profile reference, metadata, artifact paths, checksums,
    counts, capabilities, and artifact bytes during validation.
14. Refuse manual or programmatic use of an unsupported `approved` status.
15. Define deterministic behavior for revalidating a terminal package. The default is
    read-only reporting without rewriting the terminal manifest unless a separately
    approved revalidation policy is added.
16. Keep invalid-package startup policy (`fail` or `quarantine`) outside the PR 3
    builder and express it through PR 4 loading/validation services.

#### PR 4 boundaries

- Do not mutate or repair delivery or detailed artifacts.
- Do not recalculate package identity from rewritten content.
- Do not collapse valid multi-parent relationships.
- Do not infer instructional order from source export order.
- Do not introduce country- or curriculum-specific conditionals.
- Do not add a graph store, traversal API, catalog, search service, FastMCP application,
  tool, resource, or prompt.
- Do not expose arbitrary local paths in public errors.
...
- Automated tests remain deferred until explicitly requested. Until then, run
  formatting, linting, full type checking, compilation, all-six-package positive
  validation smoke checks, and representative negative checks.

#### PR 4 acceptance checks

- all six pending packages load through one generic repository and validator;
- exact manifest, profile, and artifact checksums are verified from packaged bytes;
- every declared path remains safe and package-relative;
- undeclared or malformed package content is rejected;
- decoded counts and capabilities equal the manifest;
- all endpoints resolve and endpoint representations agree;
- duplicate identifiers, unsupported duplicate edges, self-loops, cycles, and
  unreachable items are handled according to explicit generic policy;
- India Science preserves all valid multi-parent edges and passes;
- the 15 `unresolvedRootFallback` relationships remain visible and classified;
- codeless packages pass without false code-related failures;
- terminal status is one of `passed`, `failed`, or `quarantined`;
- terminal persistence sets `validatedAt` and does not change `createdAt`;
- no `approved` status is introduced;
- no artifact bytes or package-defining manifest fields are changed;
- public failures do not expose arbitrary local paths;
- no country- or curriculum-specific conditional is added to generic Python.

## 37. Packaging current curriculum artifacts

PR 3 provides a Typer-based manifest-build CLI owned by KGForEdGlobalMCP. The
application currently exposes one command, so no `build` subcommand is required.

From the repository root, view help with:

```bash
uv --directory backend run python -m kgfegmcp.cli.build_manifests --help
```

The safest first operation is a dry-run from one build specification:
...
and prints proposed output paths without creating or modifying package files. Its
`createdAt` is provisional and may differ from materialization.

Materialize the package by omitting `--dry-run`:
...
A repeated build against an equivalent pending package returns
`existing_identical`, preserves the existing manifest and `createdAt`, and performs no
writes. A conflicting, malformed, incomplete, or terminal destination fails and is never
overwritten.
...
`--spec` cannot be combined with explicit package inputs. `--project-dir` and
`--dry-run` are execution controls and may accompany specification mode.

The CLI must:

1. resolve the selected profile beneath the configured central profile root;
2. decode delivery artifacts through the existing decoder;
3. locate exactly one framework root;
4. compare deterministic framework-root metadata with the selected profile;
5. require the explicit authoritative version token;
6. preserve original safe `as_` artifact filenames and exact bytes;
7. discover only recognized detailed artifacts;
8. accept nonstandard artifacts only through explicit unique logical names;
9. calculate exact artifact and profile checksums;
10. derive counts and capabilities from profile policy and decoded evidence;
11. calculate the canonical artifact-set hash and immutable identities;
12. construct the existing pending manifest;
13. stage, verify, and atomically materialize the candidate package;
14. never overwrite or mutate an existing destination;
15. print a structured JSON build result.

### Manual review and validation lifecycle

Operators should review a generated pending manifest and package as a human sanity check.
Review should confirm the selected profile, framework metadata, rights, source version,
artifact names, counts, capabilities, unresolved evidence, and multi-parent evidence.

Manual review does not authorize editing:
...
Do not change the status to `approved`; that value is not part of the established
validation vocabulary. Fix incorrect source artifacts, profiles, or build inputs and
construct a new candidate when necessary.

PR 4 will provide the full validation CLI and owns the controlled transition:
...
PR 4 must set `validatedAt` for a terminal state, retain the original `createdAt`, and
leave package identity, package-defining manifest fields, and artifact bytes unchanged.

Manifest construction success means only that the candidate package description is
internally coherent and reproducible. It does not replace complete endpoint,
uniqueness, topology, reachability, profile-semantic, and detailed-report validation.
...
Accepted. Manifest building and the next package-validation CLI use Typer entry points.
Domain behavior remains in ordinary services so CLI concerns do not leak into package
...
| Package revision | Exactly 1 in PR 3 |
...
| Validation lifecycle | PR 3 creates `pending`; PR 4 sets `passed`, `failed`, or `quarantined` |
| Package CLI | Typer |
