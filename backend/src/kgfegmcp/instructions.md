...
derivation, not source-authored terminology. Package validation must compare it with the
selected profile's expected mapping. The loader must preserve mismatches as validation
failures rather than silently rewriting source graph records.
...
7. **PR 7:** FastMCP bootstrap, lifespan, explicit registration, logging, and error
   translation.
8. **PR 8:** canonical framework and standards tools.
9. **PR 9:** resources and strict URI resolver.
10. **PR 10:** role-oriented prompts and prompt snapshots.
11. **PR 11:** comparison evidence services.
12. **PR 12:** Claude Desktop packaging and STDIO smoke coverage.
13. **Later PRs:** alignment overlay, Learning Components, Learning Progressions, and
    remote hosting.
